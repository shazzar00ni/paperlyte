...
export function getPerformanceTargets() {
  ...
  try {
    ...
    return {
      lighthouse: {
        performance:
          (assertions['categories:performance']?.[1]?.minScore || 0.9) * 100,
        accessibility:
          (assertions['categories:accessibility']?.[1]?.minScore || 1.0) * 100,
        bestPractices:
          (assertions['categories:best-practices']?.[1]?.minScore || 0.95) *
          100,
      ...
      coreWebVitals: {
        ...
        lcp:
          assertions['largest-contentful-paint']?.[1]?.maxNumericValue || 2500,
